                ...
                <h2 class="accordion-header" id="heading{{ $i }}">
                    <div class="d-flex align-items-center">
                        <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapse{{ $i }}" aria-expanded="false" aria-controls="collapse{{ $i }}">
                            <span class="me-2"><h5 class="event-name">{{ $i + 1 }} {{ $event->name }}</h5></span>
                        </button>
                    </div>
                </h2>
                    ...
                    <div class="accordion-body">
                        <div class="row">
                            <div class="col-md-3 mb-3 d-flex justify-content-center align-items-center">
                                <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#addImageModal" data-event-id="{{ $event->id }}">Add New</button>
                            </div>
                            
                            @foreach ($event->galleries as $image)
                            <div class="col-md-3 mb-3 border position-relative">
                                <div class="image-container" style="background-image: url('{{ asset('/assets/admin/images/events/' . $image->image) }}');height:200px">
                                    <div class="image-actions d-flex flex-row px-2 py-1">
                                        @if($image->status==1)
                                        <a href="/admin/gallery/op/{{$image->id}}" class="btn btn-warning btn-sm me-2">Deactive</a>
                                        @else
                                        <a href="/admin/gallery/op/{{$image->id}}" class="btn btn-success btn-sm me-2">Active</a>
                                        @endif
                                        <a href="/admin/gallery/delete/{{$image->id}}" class="btn btn-danger btn-sm ms-auto">Delete</a>
                                    </div>
                                </div>
                            </div>
                            @endforeach
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    $('#searchInput').on('keyup', function () {
        ...
        $('.accordion-item').filter(function () {
            $(this).toggle($(this).find('.event-name').text().toLowerCase().indexOf(value) > -1);
        });
    });
    });
</script>
